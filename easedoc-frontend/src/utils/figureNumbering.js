...
const SECTION_CAPTION_HINTS = [
  ...
  { match: /architecture viewpoint/i, caption: "Viewpoint illustration" },
  { match: /system architecture/i, caption: "System architecture diagram" },
  { match: /architectural design/i, caption: "Architectural design diagram" },
  { match: /interface design/i, caption: "Interface diagram" },
  { match: /external interface/i, caption: "External interface diagram" },
  { match: /stakeholder|system context/i, caption: "System context diagram" },
  { match: /data design/i, caption: "Data model diagram" },
  { match: /design overview/i, caption: "Design overview diagram" },
  { match: /deployment/i, caption: "Deployment diagram" },
];
...
export const parseSeedBlocks = (seedBlocks) => {
  if (!seedBlocks) return null;

  try {
    const parsed =
      typeof seedBlocks === "string" ? JSON.parse(seedBlocks) : seedBlocks;
